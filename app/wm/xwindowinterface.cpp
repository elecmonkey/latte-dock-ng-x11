...
WindowId widToWindowId(WId wid)
{
    return QByteArray::number(static_cast<qulonglong>(wid));
}

WId windowIdToWId(const WindowId &wid)
{
    bool ok{false};
    const qulonglong value = wid.toULongLong(&ok);
    return ok ? static_cast<WId>(value) : 0;
}
...
xcb_window_t appRootWindow()
{
    ...
    return iterator.data ? iterator.data->root : static_cast<xcb_window_t>(XCB_WINDOW_NONE);
}
    ...
    : AbstractWindowInterface(parent)
{
    ...
    connect(KX11Extras::self(), &KX11Extras::activeWindowChanged, this, [this](WId wid) {
        Q_EMIT activeWindowChanged(widToWindowId(wid));
    });

    connect(KX11Extras::self(), &KX11Extras::windowRemoved, this, [this](WId wid) {
        Q_EMIT windowRemoved(widToWindowId(wid));
    });
...
void XWindowInterface::setViewExtraFlags(QObject *view,bool isPanelWindow, Latte::Types::Visibility mode)
{
    ...
    //! Layer to be applied
    const WindowId id = widToWindowId(winId);
    if (mode == Latte::Types::WindowsCanCover || mode == Latte::Types::WindowsAlwaysCover) {
        setKeepBelow(id, true);
    } else if (mode == Latte::Types::NormalWindow) {
        setKeepBelow(id, false);
        setKeepAbove(id, false);
    } else {
        setKeepAbove(id, true);
    }
}
...
{
    KX11Extras::setOnActivities(windowIdToWId(wid), activities);
}
...
{
    return widToWindowId(KX11Extras::activeWindow());
}
...
{
    NETWinInfo ni(x11Connection(), windowIdToWId(wid), appRootWindow(), NET::Properties(), NET::WM2GTKFrameExtents);
...
void XWindowInterface::setFrameExtents(QWindow *view, const QMargins &margins)
{
    ...
    NETWinInfo ni(x11Connection(), view->winId(), appRootWindow(), NET::Properties(), NET::WM2GTKFrameExtents);
  ...
  /*NETWinInfo ni2(x11Connection(), view->winId(), appRootWindow(), 0, NET::WM2GTKFrameExtents);
...
{
    const WId x11Wid = windowIdToWId(wid);
    const KWindowInfo winfo{x11Wid, NET::WMFrameExtents
                | NET::WMWindowType
                | NET::WMGeometry
                | NET::WMDesktop
                | NET::WMState
    ...
    if (!winfo.valid() || plasmaBlockedWindow) {
        ...
        winfoWrap.setParentId(widToWindowId(winfo.transientFor()));
        winfoWrap.setIsActive(KX11Extras::activeWindow() == x11Wid);
...
{
    const WId x11Wid = windowIdToWId(wid);
    const KWindowInfo info(x11Wid, NET::Properties(), NET::WM2WindowClass | NET::WM2DesktopFileName);
    ...
    return windowUrlFromMetadata(info.windowClassClass(),
                                 NETWinInfo(x11Connection(), x11Wid, appRootWindow(), NET::WMPid, NET::Properties2()).pid(),
...
{
    KX11Extras::activateWindow(windowIdToWId(wid));
}
...
QIcon XWindowInterface::iconFor(WindowId wid)
{
    ...
    icon.addPixmap(KX11Extras::icon(x11Wid, KIconLoader::SizeSmall, KIconLoader::SizeSmall, false));
    icon.addPixmap(KX11Extras::icon(x11Wid, KIconLoader::SizeSmallMedium, KIconLoader::SizeSmallMedium, false));
    icon.addPixmap(KX11Extras::icon(x11Wid, KIconLoader::SizeMedium, KIconLoader::SizeMedium, false));
    icon.addPixmap(KX11Extras::icon(x11Wid, KIconLoader::SizeLarge, KIconLoader::SizeLarge, false));
...
void XWindowInterface::requestClose(WindowId wid)
{
    ...
    ri.closeWindowRequest(windowIdToWId(wInfo.wid()));
}
...
void XWindowInterface::requestMoveWindow(WindowId wid, QPoint from)
{
    ...
    ri.moveResizeRequest(windowIdToWId(wInfo.wid()), validX, validY, NET::Move);
}
...
void XWindowInterface::requestToggleIsOnAllDesktops(WindowId wid)
{
    ...
    if (wInfo.isOnAllDesktops()) {
        KX11Extras::setOnDesktop(windowIdToWId(wid), KX11Extras::currentDesktop());
        KX11Extras::forceActiveWindow(windowIdToWId(wid));
    } else {
        KX11Extras::setOnAllDesktops(windowIdToWId(wid), true);
    }
}
...
void XWindowInterface::requestToggleKeepAbove(WindowId wid)
{
    ...
    NETWinInfo ni(x11Connection(), windowIdToWId(wid), appRootWindow(), NET::WMState, NET::Properties2());
...
void XWindowInterface::setKeepAbove(WindowId wid, bool active)
{
    ...
    if (active) {
        KX11Extras::setState(x11Wid, NET::KeepAbove);
        KX11Extras::clearState(x11Wid, NET::KeepBelow);
    } else {
        KX11Extras::clearState(x11Wid, NET::KeepAbove);
    }
}
...
void XWindowInterface::setKeepBelow(WindowId wid, bool active)
{
    ...
    if (active) {
        KX11Extras::setState(x11Wid, NET::KeepBelow);
        KX11Extras::clearState(x11Wid, NET::KeepAbove);
    } else {
        KX11Extras::clearState(x11Wid, NET::KeepBelow);
    }
...
void XWindowInterface::requestToggleMinimized(WindowId wid)
{
    ...
    if (wInfo.isMinimized()) {
        ...
        KX11Extras::unminimizeWindow(windowIdToWId(wid));

        if (onCurrent) {
            KX11Extras::forceActiveWindow(windowIdToWId(wid));
        }
    } else {
        KX11Extras::minimizeWindow(windowIdToWId(wid));
    }
}
...
{
    const KWindowInfo info(windowIdToWId(wid), NET::WMGeometry | NET::WMState, NET::WM2WindowClass);
...
{
    const WindowId id = widToWindowId(wid);
    if (!isAcceptableWindow(id)) {
        return;
    }

    emit windowAdded(id);
    considerWindowChanged(id);
}
